...
```bash
Ответ:
Iaac позволяет описать свою систему в виде кода, с помощью которого в дальнейшем можно будет быстро разворачивать идентичные системы. Iaac позволяет избежать дрейфа конфигурации.
Основополагающий принцип iaac это скорость развёртывания и возможность легкого масштабирования инфраструктуры
```
...
На мой взгляд, следует использовать то, что больше подходит к конкретному случаю или комбинировать. Если делать выбор в пользу одного, думаю что я бы выбрал push т.к. pull более трудоемкий и проигрывает по безопасности.
```
...
```bash
Ответ:
-DESKTOP-QQQ9C8Q,Oracle VM VirtualBox 6.1.30,6.1.30

-C:\Users\ki4eg>vagrant --version
Vagrant 2.2.19
...
```
4. Воспроизвести практическую часть лекции самостоятельно. Создать виртуальную машину. Зайти внутрь ВМ, убедиться, что Docker установлен с помощью команды docker ps
```bash
